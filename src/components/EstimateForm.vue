    ...
    <v-row justify="center">
      <v-col cols="12" lg="10" xl="8">
        <div class="d-flex align-center mb-6">
          ...
          <h1 class="text-h4 font-weight-black teal--text text--darken-3">
            Estimate Creator
          </h1>
          <v-spacer></v-spacer>
        ...
        <v-form ref="form" v-model="valid">
          <v-card class="mb-8 pa-8 rounded-xl elevation-2 border-top-teal">
            <div
              class="text-subtitle-1 font-weight-bold mb-6 grey--text text--darken-2"
            >
              <v-icon left color="teal">mdi-information</v-icon> Project
              Information
            </div>
            <v-row>
              <v-col cols="12" md="4">
              ...
              <v-col cols="12" md="4">
                <v-autocomplete
                  v-model="estimate.billType"
                  :items="['ESTIMATE', 'BILL', 'TAX INVOICE']"
                  label="Bill Type"
                  flat
                  solo-inverted
                  hide-details
          ...
          >
            ...
            <v-card class="rounded-xl overflow-hidden elevation-1 bg-white">
              <div
                ...
                :key="itemIndex"
                class="pa-6 position-relative border-bottom"
              >
                <v-row align="center">
                  <v-col cols="12" md="4">
                    <v-text-field
          ...
          <v-btn
            ...
            dashed
            color="teal"
            class="rounded-xl border-dashed mb-10"
            @click="addCategory"
            outlined
          >
...
export default {
  ...
  computed: {
    ...
    grandTotal() {
      return this.estimate.categories.reduce((sum, category) => {
        if (category.isOption) return sum;
        const categoryTotal = category.items.reduce((itemSum, item) => {
          if (item.isOption) return itemSum;
  ...
  methods: {
    ...
    duplicateCategory(categoryIndex) {
      const categoryToCopy = this.estimate.categories[categoryIndex];
      // Deep clone using JSON method
      const clonedCategory = JSON.parse(JSON.stringify(categoryToCopy));
      // Insert right after the original
      this.estimate.categories.splice(categoryIndex + 1, 0, clonedCategory);
    },
